...
function Path(points, odMatrix) {
    ...
    for (var i = 0; i < points.length; i++)
        for (let j = 0; j < points.length; j++)
            this.distances[j + i * points.length] = odMatrix
                ? odMatrix[i][j]
 ...
 * @param {Number[][]} [odMatrix=] OD Matrix in case user already have it
 ...
 **/
function solve(points, odMatrix, temp_coeff, callback) {
    let path = new Path(points, odMatrix);
